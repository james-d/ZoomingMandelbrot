...
public abstract class FractalView {
    ...
    private static final int PARALLELIZATION_LEVEL = 
        ...
        for (int strip = 0; strip < numStrips; strip++) {
            ...
            exec.execute(() -> {
                try {
                    ...
                    pw.setPixels(0, startRow, width, endRow-startRow, PixelFormat.getIntArgbInstance(), pixels, 0, width);
                } finally {
                    latch.countDown();
                }
            });
    ...
    private int computePixel(double cx, double cy) {
        ...
        int index = maxIterations >= PALETTE.length ? iterations % PALETTE.length : (PALETTE.length * iterations) / maxIterations ;
