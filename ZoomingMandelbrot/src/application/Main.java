...
public class Main extends Application {
	...
	private void setUpKeyboardShortcuts(Scene scene, MandelbrotExplorerController controller) {
	    ...
	    List<KeyboardAction> actions = Arrays.asList(
                ...
                new KeyboardAction(Platform::exit,
                        KeyCode.Q, KeyCombination.SHORTCUT_DOWN)
	            
	    );
	    
	    scene.setOnKeyPressed(e -> actions.forEach(action -> action.runIfMatches(e)));
	...
	private static class KeyboardAction {
	    ...
	    KeyboardAction(Runnable action, KeyCode keyCode, KeyCombination.Modifier... modifiers) {
	        this.key = new KeyCodeCombination(keyCode, modifiers);
	        this.action = action ;
	    }
	    
	    void runIfMatches(KeyEvent event) {
	        if (key.match(event)) action.run();
	    }
	}
