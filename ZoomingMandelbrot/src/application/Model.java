...
public class Model {
    ...
    public final void setCurrentMandelbrot(
            final MandelbrotView currentMandelbrot) {
    ...
    public void startZoom(double mouseX, double mouseY, double zoomFactor, Optional<Integer> maxIterations) {
        ...
        final double deltaX = bounds.getWidth() * (mouseX / VIEW_WIDTH - 0.5); // targetX - currentX ;
        final double deltaY = bounds.getHeight() * (1 - mouseY / VIEW_HEIGHT - 0.5) ; // targetY - currentY ;
            ...
            public Void call() {
                ...
                for (int i = 1; i <= ANIMATION_FRAMES; i++) {
                    try {
                        ...
                        viewQueue.put(createMandelbrotView(currentX + i * frameDeltaX,
    ...
    private MandelbrotView createMandelbrotView(double centerX, double centerY, 
        ...
        MandelbrotView mandelbrot = new MandelbrotView(VIEW_WIDTH, VIEW_HEIGHT, bounds, maxIterations, exec);
        mandelbrot.compute();
    ...
    public void computeJuliaSet(double cx, double cy, int iterationLevel, Runnable whenFinished) {
        exec.execute(() -> {
            JuliaSetView juliaSet = new JuliaSetView(VIEW_WIDTH, VIEW_HEIGHT, iterationLevel, exec, cx, cy);
            juliaSet.compute();
