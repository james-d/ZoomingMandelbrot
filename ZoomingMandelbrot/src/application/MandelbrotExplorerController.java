...
public class MandelbrotExplorerController {
    ...
    private void initialize() {
        model.currentMandelbrotProperty().addListener((obs, oldMandelbrot, newMandelbrot) -> 
            mandelbrotView.setImage(newMandelbrot.getImage()));
        model.currentJuliaSetProperty().addListener((obs, oldJulia, newJulia) -> 
            juliaView.setImage(model.getCurrentJuliaSet().getImage()));
        mandelbrotView.disableProperty().bind(model.zoomingInProgressProperty());
    ...
    private void bindZoomingRectangles() {
        ChangeListener<Number> updateZoomRect = (obs, oldValue, newValue) -> {
            if (dragRect.getHeight() < dragRect.getWidth()) {
                ...
                zoomRect.setY(dragRect.getY() - (dragRect.getWidth() - dragRect.getHeight()) / 2);
                zoomRect.setHeight(dragRect.getWidth());
            } else {
                zoomRect.setY(dragRect.getY());
                zoomRect.setHeight(dragRect.getHeight());
                zoomRect.setX(dragRect.getX() - (dragRect.getHeight() - dragRect.getWidth()) / 2);
    ...
    private void setUpJuliaSetTracking() {
        ...
        mandelbrotView.setOnMouseMoved(e -> {
            if (model.isTrackingJuliaSet() && juliaComputing.compareAndSet(false, true)) {
                ...
                model.computeJuliaSet(cx, cy, 50, () -> juliaComputing.set(false));
            }
        });
    }
        ...
        AnimationTimer viewUpdater = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                if (mandelbrot != null) {
                    ...
                    model.setFramesPendingRendering(model.getFramesPendingRendering() - 1);
